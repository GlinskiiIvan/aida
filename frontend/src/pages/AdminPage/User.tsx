...
const UserPage = () => {
  ...
  const columns: ColumnTable<(typeof allData)[number]>[] = [
    ...
    {
      key: "banned",
      label: t("entities.user.fields.banned"),
      sortable: false,
      render: (item) => (item.banned ? <span>Да</span> : <span>Нет</span>),
    },
  ...
  React.useEffect(() => {
    if (allUserRolesData.isSuccess) {
      setUserRoles(allUserRolesData.data.data ?? []);
    }
            ...
            <Stack direction="column" gap="md" justify="flex-start" align="stretch">
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, email: val }))
                }
              ...
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, password: val }))
                }
      ...
      <Modal
        ...
        footer={
          <Stack direction="row" gap="sm" justify="flex-end" align="center">
            <Button variant="primary" intent="normal" icon="CHECK" onClick={updateUserRolesHandler}>
              {t("actions.confirm")}
            </Button>
            <Button
              variant="secondary"
              intent="destructive"
              icon="CLOSE"
              onClick={() => updateUserRolesModal.close()}
            >
              {t("actions.cancel")}
            </Button>
          </Stack>
      ...
      >
        {allRolesData.isSuccess && (
          <Select<Role>
            ...
            options={allRolesData.data.data}
            value={userRoles}
            onChangeValue={setUserRoles}
            getKey={(T) => T.id}
            getValue={(T) => T.value}
          />
        )}
