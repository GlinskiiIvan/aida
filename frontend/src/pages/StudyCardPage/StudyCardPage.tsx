...
const StudyCardPage = () => {
  ...
  } = studyApi.useFindOneStudyQuery(Number(id));
  const [
    remove,
    {
      isSuccess: studyRemoveIsSuccess,
      isError: studyRemoveIsError,
      error: studyRemoveError,
      fulfilledTimeStamp: studyRemoveFulfilledTimeStamp,
    },
  ] = studyApi.useRemoveStudyMutation();
  const [
    update,
    {
      isSuccess: studyUpdateIsSuccess,
      isError: studyUpdateIsError,
      error: studyUpdateError,
      fulfilledTimeStamp: studyUpdateFulfilledTimeStamp,
    },
    ...
    if (studyIsSuccess && study.status === "completed") {
      setStudyInfoList([
        [
          ...
          {
            key: t("entities.study.fields.specificCharacterSet"),
            value: study?.specificCharacterSet,
          },
          {
            key: t("entities.study.fields.studyDateTime"),
            value: formatedDate(study?.studyDateTime || null),
          },
        ...
        [
          ...
          {
            key: t("entities.study.fields.manufacturersModelName"),
            value: study?.manufacturersModelName,
          },
  ...
  React.useEffect(() => {
    if (studyUpdateIsSuccess) {
      dispatch(
        addNotice({ type: "success", message: t(`notification.success.updated.default.singular`) }),
      );
      refetch();
    }
  ...
  React.useEffect(() => {
    if (studyRemoveIsSuccess) {
      dispatch(
        addNotice({ type: "success", message: t(`notification.success.deleted.default.singular`) }),
      );
  ...
  const canSubmitPredictionRun =
    study?.status === "completed" && model !== undefined && id !== undefined;
  ...
  const columns: ColumnTable<(typeof allpredictionRunsData)[number]>[] = [
    {
      key: "model",
      label: t("entities.predictionRun.fields.model"),
      sortable: false,
      width: "100px",
    },
    {
      key: "version",
      label: t("entities.predictionRun.fields.version"),
      sortable: false,
      width: "100px",
    },
    {
      key: "status",
      label: t("entities.predictionRun.fields.status"),
      sortable: false,
      width: "100px",
    },
    {
      key: "createdAt",
      label: t("entities.predictionRun.fields.runnedAt"),
      sortable: false,
      render: (run) => formatedDate(run.createdAt),
      width: "200px",
    },
  ];
  ...
  const prevImage = () => {
    if (imagesData.isSuccess && activeImage !== undefined) {
      const currentImageIdx = imagesData.data.data.findIndex(
  ...
  const getCurrentImageRef = () => {
    return fullScreenImageModal.show ? fullScreenImageRef.current : activeImageContentRef.current;
  };
  ...
  const [predictionsTrigger, predictionsData] =
    predictionRunApi.useLazyFindAllPredictionsByRunQuery();
  ...
  React.useEffect(() => {
    ...
    if (predictionsData.isSuccess && img && activeImage) {
      const results =
        (predictionsData.data.data.find((item) => item.imageId === activeImage.id)
      ...
      {studyIsSuccess && study.status === "completed" && (
        ...
        >
          <Block
            ...
            actions={
              <Stack direction="row" gap="xs" justify="center" align="center">
                <IconButton
                  icon={{ name: "RELOAD", color: "tertiary", size: "lg" }}
                  onClick={refetch}
                />
                <IconButton
                  icon={{ name: "EDIT", color: "tertiary", size: "lg" }}
                  onClick={updateModal.open}
                />
                <IconButton
                  icon={{ name: "REMOVE", color: "tertiary", size: "lg" }}
            ...
            footer={
              <Stack direction="row" gap="md" justify="center" align="center">
                <Button variant="primary" icon="ADD" onClick={predictionModal.open}>
                  {t("actions.runPrediction")}
                </Button>
                <Button variant="secondary" icon="LINK" onClick={navigatePatientHandler}>
                  {t("actions.goToPatient")}
                </Button>
              </Stack>
            }
          >
          ...
          >
            ...
            <Block fullWidth className={styles.viewer}>
              <Stack
              ...
              >
                      ...
                      >
                        <img
                          className={clsx(styles.image, {
                            [styles.active]: isActiveImage(image.id),
                          })}
                ...
                <Stack className={styles.content}>
                  ...
                  {activeImage &&
                    ...
                    overlays.map((item) => (
                      <div
                        key={`${item.class}-${item.bbox.join("-")}`}
                        className={styles.overlay}
                        style={item.inlineStyleOverlay}
                      >
                        <span>
          ...
          <FullScreenImage title={activeImage?.imageName} options={fullScreenImageModal}>
            <Stack
              className={styles.predictionWorkspace}
              style={{
                width: "100%",
            ...
            <Stack direction="column" gap="md" justify="flex-start" align="stretch">
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, description: val }))
                }
              ...
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, institutionName: val }))
                }
              ...
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, manufacturer: val }))
                }
              ...
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, manufacturersModelName: val }))
                }
              ...
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, referringPhysiciansName: val }))
                }
              ...
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, stationName: val }))
                }
              ...
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, reason: val }))
                }
          ...
          <Modal
            ...
            footer={
              <Stack direction="row" gap="sm" justify="flex-end" align="center">
                <Button variant="primary" intent="destructive" icon="CHECK" onClick={removeHandler}>
                  {t("actions.confirm")}
                </Button>
                <Button variant="secondary" icon="CLOSE" onClick={() => removeModal.close()}>
                  {t("actions.cancel")}
                </Button>
              </Stack>
            }
          ...
          <Modal
            ...
            footer={
              <Stack direction="row" gap="sm" justify="flex-end" align="center">
                <Button variant="primary" icon="ADD" onClick={predictionRunHandler}>
                  {t("actions.runPrediction")}
                </Button>
              </Stack>
            }
