...
export const loginThunk = createAsyncThunk<AuthResponse, AuthRequest, { rejectValue: AxiosError }>(
  "/user/loginThunk",
...
export const logoutThunk = createAsyncThunk<void, void, { rejectValue: AxiosError }>(
  "/user/logoutThunk",
  async (params, { rejectWithValue }) => {
    try {
...
export const checkThunk = createAsyncThunk<AuthResponse, void, { rejectValue: AxiosError }>(
  "/user/checkThunk",
  async (params, { rejectWithValue }) => {
    try {
  ...
  extraReducers: (builder) => {
      ...
      if (action.payload) {
        state.error =
          (action.payload.response &&
            (action.payload.response?.data as { detail: string })?.detail) ||
          "Unknown error.";
      } else {
    ...
    builder.addCase(checkThunk.rejected, (state, action) => {
      ...
      if (action.payload) {
        state.error =
          (action.payload.response &&
            (action.payload.response.data as { statusCode: number; message: string })?.message) ||
          "Unknown error.";
      } else {
        state.error = "Unknown error.";
      }
