...
const doctorPage = () => {
  ...
  const oneData =
    oneDataQuery[1].data?.status === enums.ResponseStatus.SUCCESS
      ? oneDataQuery[1].data.data
      : undefined;
  ...
  const columns: ColumnTable<(typeof allData)[number]>[] = [
    ...
    {
      key: "user",
      label: t("entities.doctor.fields.user"),
      sortable: false,
      render: (el) => el.user.email,
    },
  ...
  const fillFormWithRecordData = () => {
    if (oneData) {
      setRecordFields({
        user:
          allUserData.data?.status === enums.ResponseStatus.SUCCESS
            ? allUserData.data.data.find((el) => el.id === oneData.id)
            : undefined,
          ...
          body: (
            <Stack direction="column" gap="md" justify="flex-start" align="stretch">
              <Select<User>
                ...
                options={allUserData.data?.data || []}
                value={recordFields.user}
                onChangeValue={(value) => setRecordFields({ ...recordFields, user: value })}
                getKey={(T) => T.id}
                getValue={(T) => T.email}
              />
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, fullName: val }))
                }
              ...
              <Select<(typeof gendertOptions)[0]>
                ...
                onChangeValue={(value) => setRecordFields({ ...recordFields, gender: value })}
                getKey={(T) => T.id}
                getValue={(T) => T.name}
              />
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, phone: val }))
                }
              ...
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, contactEmail: val }))
                }
              ...
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, specialization: val }))
                }
              ...
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, department: val }))
                }
              ...
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, licenseNumber: val }))
                }
