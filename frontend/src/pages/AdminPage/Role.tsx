...
const rolePage = () => {
  ...
  const [updateRolePermissionsTrigger, updateRolePermissionsData] =
    roleApi.useUpdatePermissionsMutation();

  const [allRolePermissionsTrigger, allRolePermissionsData] =
    roleApi.useLazyFindAllRolePermissionsQuery();
  const [allPermissionsTrigger, allPermissionsData] =
    permissionApi.useLazyFindAllPermissionsQuery();
  ...
  const canSubmitRecord =
    recordFields.value.trim() !== "" && recordFields.description.trim() !== "";
  ...
  React.useEffect(() => {
    if (allRolePermissionsData.isSuccess) {
      setRolePermissions(allRolePermissionsData.data.data ?? []);
    }
            ...
            <Stack direction="column" gap="md" justify="flex-start" align="stretch">
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setRecordFields((prev) => ({ ...prev, value: val }))
                }
        ...
        footer={
          <Stack direction="row" gap="sm" justify="flex-end" align="center">
            <Button
              ...
              intent="normal"
              icon="CHECK"
              onClick={updateRolePermissionsHandler}
            >
              {t("actions.confirm")}
            </Button>
            <Button
              ...
              intent="destructive"
              icon="CLOSE"
              onClick={() => updateRolePermissionsModal.close()}
            >
              {t("actions.cancel")}
            </Button>
          </Stack>
      ...
      >
        {allPermissionsData.isSuccess && (
          <Select<Permission>
            ...
            options={allPermissionsData.data.data}
            value={rolePermissions}
            onChangeValue={setRolePermissions}
            getKey={(T) => T.id}
            getValue={(T) => T.value}
          />
        )}
