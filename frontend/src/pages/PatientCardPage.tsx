...
const PatientCardPage = () => {
  ...
  } = patientApi.useFindOnePatientQuery(Number(id));
  const [
    remove,
    {
      ...
      error: removeError,
      fulfilledTimeStamp: removeFulfilledTimeStamp,
    },
  ] = patientApi.useRemovePatientMutation();
  const [
    update,
    {
      ...
      error: updateError,
      fulfilledTimeStamp: updateFulfilledTimeStamp,
    },
  ...
  if (patient?.isPublic)
    patientInfoList.push({
      key: t("entities.patient.fields.public"),
      value: <Icon name="CHECK" color="primary" size="lg" />,
    });
    ...
    if (updateModal.show) {
      if (patient && patientIsSuccess) {
        setRecordFields({
          ...
          gender:
            gendertOptions.find((option) => option.id === patient.gender) || gendertOptions[0],
          phone: patient.phone,
          email: patient.email || "",
          note: patient.note || "",
  ...
  const columns: ColumnTable<(typeof allStudiesData)[number]>[] = [
    ...
    {
      key: "studyDateTime",
      label: t("entities.study.fields.studyDateTime"),
      sortable: false,
      render: (item) => formatedDate(item.studyDateTime),
    },
  ...
  const [studyRecordFields, setStudyRecordFields] =
    React.useState<studyRecordData>(initialStudyRecordData);
  ...
  const canSubmitStudyRecord =
    studyRecordFields.patient !== undefined && studyRecordFields.dicomZip !== null;
  ...
  React.useEffect(() => {
    if (uploadStudyMutation[1].data?.task_id) {
      setTaskId(uploadStudyMutation[1].data?.task_id);
    }
  }, [uploadStudyMutation[1].data?.task_id]);

  const socket = React.useMemo(
    () => new WebSocketClient<TaskMessageIngestionStudy>(`ws://localhost:8002/ws/tasks/${taskId}`),
    [taskId],
  );
      ...
      <ManagedTable
        ...
        record={{
            ...
            <Stack direction="column" gap="md" justify="flex-start" align="stretch">
              <TextField
                ...
                onChangeValue={(val: string) =>
                  setStudyRecordFields((prev) => ({ ...prev, note: val }))
                }
                label={t("entities.study.fields.note")}
              />
              <Button variant={"secondary"} onClick={handleFileInputClick}>
                {studyRecordFields.dicomZip?.name || t("common.dicomZip")}
              </Button>
              <input
                id="dicomZip"
                ref={fileInputRef}
                type="file"
                onChange={(e) =>
                  setStudyRecordFields((prev) => ({
                    ...prev,
                    dicomZip: e.target.files !== null ? e.target.files[0] : null,
                  }))
                }
                style={{ display: "none" }}
              />
          ...
          toFormData: () => {
            ...
            studyRecordFields.patient &&
              formData.append("patientId", studyRecordFields.patient.id.toString());
      ...
      >
        ...
        <Stack direction="column" gap="md" justify="flex-start" align="stretch">
          ...
          <TextField
            ...
            onChangeValue={(val: string) =>
              setRecordFields((prev) => ({ ...prev, birthDate: val }))
            }
          ...
          <Select
            ...
            onChangeValue={(val) =>
              setRecordFields((prev) => ({ ...prev, gender: val || gendertOptions[0] }))
            }
